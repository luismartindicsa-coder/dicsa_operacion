...
class ComprasPriceAdjustmentsPage extends StatefulWidget {
  final bool instantOpen;

  const ComprasPriceAdjustmentsPage({super.key, this.instantOpen = false});

  @override
  State<ComprasPriceAdjustmentsPage> createState() =>
      _ComprasPriceAdjustmentsPageState();
}

class _ComprasPriceAdjustmentsPageState
    extends State<ComprasPriceAdjustmentsPage>
  ...
  Future<void> _loadPricingData() async {
    ...
    final history = await ComprasDataStore.loadPriceHistory();
    final companyById = <String, ComprasCatalogProviderRecord>{
      for (final row in snapshot.companies) row.id: row,
    };
    final materialById = <String, ComprasCatalogMaterialRecord>{
  ...
  Future<void> _persistPricingData(
    ...
    try {
      await ComprasDataStore.savePriceRecords(
        rowsToPersist
            .map(
  ...
  void _setupAutoRefresh() {
    ...
    _pricingRealtimeChannel = Supabase.instance.client
        .channel('compras-pricing-auto-refresh')
        .onPostgresChanges(
        ...
        .onPostgresChanges(
          event: PostgresChangeEvent.all,
          schema: 'public',
          table: 'compras_price_adjustment_history',
          callback: (_) => unawaited(_refreshPricingIfIdle()),
        )
        .subscribe();
  }
  ...
  Future<void> _exportClientPdfReport() async {
    ...
    try {
      ...
      final baseDirectory = await FilePicker.platform.getDirectoryPath(
        dialogTitle: 'Seleccionar carpeta base para reportes de Compras',
      ...
      final reportsRoot = Directory(
        '$baseDirectory${Platform.pathSeparator}Precios compras $dateSuffix',
      );
      await reportsRoot.create(recursive: true);

      final accent = const PdfColor.fromInt(0xFF9C211B);
      final softAccent = const PdfColor.fromInt(0xFFF4E3E1);
      final border = const PdfColor.fromInt(0xFFD7B4B0);
      final text = const PdfColor.fromInt(0xFF241515);
      var writtenCount = 0;
  ...
  void _showStub(String label) {
    _toast('$label quedará conectado en la siguiente fase de Compras.');
  }
    ...
    return ContractGlassCard(
      ...
      child: Column(
        ...
        children: [
          ...
          const _AdjustmentSectionTitle('1. Proveedor y material'),
            ...
            child: rows.isEmpty
                ? const Center(
                    child: Padding(
                      ...
                      child: Text(
                        'No hay precios de compra para ese criterio.',
                      ),
                    ),
                  )
                : ListView.separated(
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: selected
                ? comprasAreaTokens.primaryStrong.withValues(alpha: 0.36)
                : comprasAreaTokens.border.withValues(alpha: 0.76),
          ),
        ),
        child: Text(
          label,
...
class _AdjustmentMiniPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final background = highlighted
        ...
        : comprasAreaTokens.badgeBackground.withValues(alpha: 0.74);
    final foreground = highlighted
        ? comprasAreaTokens.primaryStrong
        : comprasAreaTokens.badgeText;
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(999),
        border: Border.all(
          color: comprasAreaTokens.border.withValues(alpha: 0.84),
        ),
      ),
      child: Text(
        label,
        ...
        children: [
          ...
          Text(
            ...
            style: TextStyle(
              ...
              color: comprasAreaTokens.badgeText.withValues(alpha: 0.82),
            ),
          ),
    ...
    return MouseRegion(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: widget.active
                ? comprasAreaTokens.primaryStrong.withValues(alpha: 0.52)
                : widget.selected
                ? comprasAreaTokens.primaryStrong.withValues(alpha: 0.34)
          ...
          boxShadow: [
            BoxShadow(
              color: comprasAreaTokens.primaryStrong.withValues(
                alpha: widget.active
                    ? 0.18
                    : widget.selected
                    ? 0.12
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(
                        color: widget.selected
                            ...
                            : comprasAreaTokens.border.withValues(alpha: 0.78),
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const _MayoreoPriceSectionHeader(label: 'AREA'),
              const SizedBox(height: 8),
              Container(
                ...
                child: Column(
                  children: [
                    _MayoreoPriceNavItem(
                      ...
                      title: 'Tickets Compras',
                      subtitle: 'Captura y seguimiento operativo',
                      onTapSync: () => onNavigate('Tickets Compras'),
                    ),
                    const SizedBox(height: 8),
                    _MayoreoPriceNavItem(
                      icon: Icons.request_quote_rounded,
                    ...
                    _MayoreoPriceNavItem(
                      ...
                      title: 'Directorio Proveedores',
                      subtitle: 'Crédito, contacto y operación',
                      onTapSync: () => onNavigate('Directorio Proveedores'),
                    ),
                  ],
                ),
              ),
