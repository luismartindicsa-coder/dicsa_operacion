...
alter table public.documental_records
  add column audit_result text not null default '' check (audit_result in ('','Con observaciones','Conforme','No aplica','No conforme')),
  add column audit_findings text not null default '',
  add constraint documental_audit_organization_check check(category<>'auditorias' or length(btrim(authority))>0);
...
declare
  ...
  v_audit_result text:='';
  v_audit_findings text:='';
  v_maintenance_subject text:='';
  v_scheduled date;
  v_performed date;
...
begin
  ...
  if v_category not in ('documentacion-legal','permisos-y-tramites','seguridad-e-higiene','medio-ambiente','vehiculos','personal','proteccion-civil','contratos','seguros','mantenimiento','auditorias') then
  ...
  if v_category in ('permisos-y-tramites','seguridad-e-higiene','medio-ambiente','vehiculos','personal','proteccion-civil','contratos','seguros','mantenimiento','auditorias') then
  ...
  if v_category in ('seguridad-e-higiene','mantenimiento') then
  ...
  if v_category='mantenimiento' then
    if coalesce(p_record->>'document_type','') not in ('Certificado','Contrato','Evidencia','Inspección periódica','Mantenimiento obligatorio','Programa') then
      raise exception 'Selecciona un tipo de registro de mantenimiento válido.' using errcode='23514';
    end if;
    v_maintenance_subject:=btrim(coalesce(p_record->>'maintenance_subject',''));
    if v_maintenance_subject='' then raise exception 'Indica el equipo, instalación o alcance.' using errcode='23514'; end if;
  end if;
  if v_category in ('mantenimiento','auditorias') then
    v_scheduled:=nullif(p_record->>'scheduled_date','')::date;
    v_performed:=nullif(p_record->>'performed_date','')::date;
  end if;
  if v_category='auditorias' then
    if coalesce(p_record->>'document_type','') not in ('Certificación','Externa','Interna','Regulatoria','Seguimiento') then
      raise exception 'Selecciona un tipo de auditoría válido.' using errcode='23514';
    end if;
    if btrim(coalesce(p_record->>'authority',''))='' then
      raise exception 'Indica el organismo o auditor.' using errcode='23514';
    end if;
    v_audit_result:=btrim(coalesce(p_record->>'audit_result',''));
    if v_audit_result not in ('','Con observaciones','Conforme','No aplica','No conforme') then
      raise exception 'Selecciona un resultado válido.' using errcode='23514';
    end if;
    v_audit_findings:=btrim(coalesce(p_record->>'audit_findings',''));
  end if;
  ...
  on conflict(id) do update set title=excluded.title,document_type=excluded.document_type,responsible_user_id=excluded.responsible_user_id,
    ...
    counterparty_name=excluded.counterparty_name,signature_date=excluded.signature_date,renewal_type=excluded.renewal_type,renewal_date=excluded.renewal_date,renewal_notes=excluded.renewal_notes,insured_subject=excluded.insured_subject,coverage_description=excluded.coverage_description,maintenance_subject=excluded.maintenance_subject,scheduled_date=excluded.scheduled_date,performed_date=excluded.performed_date,audit_result=excluded.audit_result,audit_findings=excluded.audit_findings,revision=v_revision,updated_at=now(),updated_by=auth.uid();
...
begin
  ...
  if p_category not in ('documentacion-legal','permisos-y-tramites','seguridad-e-higiene','medio-ambiente','vehiculos','personal','proteccion-civil','contratos','seguros','mantenimiento','auditorias') or p_category is null then raise exception 'Categoría no habilitada.' using errcode='23514'; end if;
  with filtered as materialized (
    select id,category,title,document_type,authority,department,progress_percentage,related_employee_id,related_employee_name,study_type,provider_name,periodicity,authorization_number,installation_name,vehicle_id,vehicle_label,counterparty_name,signature_date,renewal_type,renewal_date,renewal_notes,insured_subject,coverage_description,maintenance_subject,scheduled_date,performed_date,audit_result,audit_findings,responsible_user_id,reference,status,priority,issue_date,start_date,expiration_date,revision,created_at
    from public.documental_records r
    where category=p_category
      and (coalesce(p_search,'')='' or concat_ws(' ',title,reference,authority,department,related_employee_name,provider_name,study_type,periodicity,authorization_number,installation_name,vehicle_id,vehicle_label,counterparty_name,renewal_notes,insured_subject,coverage_description,maintenance_subject,scheduled_date,performed_date,audit_result,audit_findings,case when category='auditorias' then next_action else '' end) ilike '%'||p_search||'%')
    ...
    select * from filtered order by
      ...
      case when p_sort='scheduled' then scheduled_date end asc nulls last,
