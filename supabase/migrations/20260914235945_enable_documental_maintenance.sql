...
alter table public.documental_records
  add column maintenance_subject text not null default '',
  add column scheduled_date date,
  add column performed_date date,
  add constraint documental_maintenance_subject_check check(category<>'mantenimiento' or length(btrim(maintenance_subject))>0);
...
begin
  ...
  if v_category not in ('documentacion-legal','permisos-y-tramites','seguridad-e-higiene','medio-ambiente','vehiculos','personal','proteccion-civil','contratos','seguros','mantenimiento') then
  ...
  if v_category in ('permisos-y-tramites','seguridad-e-higiene','medio-ambiente','vehiculos','personal','proteccion-civil','contratos','seguros','mantenimiento') then
