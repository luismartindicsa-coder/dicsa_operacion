...
alter table public.documental_records
  ...
  add column renewal_type text not null default '' check (renewal_type in ('','Automática','No aplica','Por acuerdo')),
  add column renewal_date date,
  add column renewal_notes text not null default '',
  add constraint documental_contract_counterparty_check check(category<>'contratos' or length(btrim(counterparty_name))>0),
  add constraint documental_renewal_date_check check(renewal_type<>'No aplica' or renewal_date is null);
...
begin
  ...
  if v_category not in ('documentacion-legal','permisos-y-tramites','seguridad-e-higiene','medio-ambiente','vehiculos','personal','proteccion-civil','contratos') then
  ...
  if v_category in ('permisos-y-tramites','seguridad-e-higiene','medio-ambiente','vehiculos','personal','proteccion-civil','contratos') then
  ...
  on conflict(id) do update set title=excluded.title,document_type=excluded.document_type,responsible_user_id=excluded.responsible_user_id,
    ...
    counterparty_name=excluded.counterparty_name,signature_date=excluded.signature_date,renewal_type=excluded.renewal_type,renewal_date=excluded.renewal_date,renewal_notes=excluded.renewal_notes,revision=v_revision,updated_at=now(),updated_by=auth.uid();
...
begin
  ...
  if p_category not in ('documentacion-legal','permisos-y-tramites','seguridad-e-higiene','medio-ambiente','vehiculos','personal','proteccion-civil','contratos') or p_category is null then raise exception 'Categoría no habilitada.' using errcode='23514'; end if;
  with filtered as materialized (
    select id,category,title,document_type,authority,department,progress_percentage,related_employee_id,related_employee_name,study_type,provider_name,periodicity,authorization_number,installation_name,vehicle_id,vehicle_label,counterparty_name,signature_date,renewal_type,renewal_date,renewal_notes,responsible_user_id,reference,status,priority,issue_date,start_date,expiration_date,revision,created_at
    from public.documental_records r
    where category=p_category
      and (coalesce(p_search,'')='' or concat_ws(' ',title,reference,authority,department,related_employee_name,provider_name,study_type,periodicity,authorization_number,installation_name,vehicle_id,vehicle_label,counterparty_name,renewal_notes) ilike '%'||p_search||'%')
