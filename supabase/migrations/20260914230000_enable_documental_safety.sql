...
alter table public.documental_records
  add column related_employee_id uuid references public.employees(id) on delete set null,
  add column related_employee_name text not null default '',
  add column study_type text not null default '',
  add column provider_name text not null default '',
  add column periodicity text not null default '';
...
begin
  ...
  if v_category not in ('documentacion-legal','permisos-y-tramites','seguridad-e-higiene') then
  ...
  if v_category in ('permisos-y-tramites','seguridad-e-higiene') then
  ...
  if v_category='seguridad-e-higiene' then
    ...
    if v_employee is not null then
      ...
      where e.id=v_employee and (e.is_active or r.related_employee_id=v_employee);
      if not found then raise exception 'Selecciona un trabajador activo.' using errcode='23514'; end if;
    end if;
  ...
  insert into public.documental_records(id,category,title,document_type,responsible_user_id,department,authority,reference,status,priority,
    issue_date,start_date,expiration_date,observations,next_action,revision,created_by,updated_by,progress_percentage,related_employee_id,related_employee_name,study_type,provider_name,periodicity)
  values(p_id,v_category,btrim(p_record->>'title'),p_record->>'document_type',(p_record->>'responsible_user_id')::uuid,
    ...
    nullif(p_record->>'expiration_date','')::date,coalesce(p_record->>'observations',''),coalesce(p_record->>'next_action',''),v_revision,auth.uid(),auth.uid(),v_progress,v_employee,v_employee_name,v_study,v_provider,v_periodicity)
  on conflict(id) do update set title=excluded.title,document_type=excluded.document_type,responsible_user_id=excluded.responsible_user_id,
    ...
    related_employee_id=excluded.related_employee_id,related_employee_name=excluded.related_employee_name,study_type=excluded.study_type,provider_name=excluded.provider_name,periodicity=excluded.periodicity,revision=v_revision,updated_at=now(),updated_by=auth.uid();
...
begin
  ...
  if p_category not in ('documentacion-legal','permisos-y-tramites','seguridad-e-higiene') or p_category is null then raise exception 'Categoría no habilitada.' using errcode='23514'; end if;
  with filtered as materialized (
    select id,category,title,document_type,authority,department,progress_percentage,related_employee_id,related_employee_name,study_type,provider_name,periodicity,responsible_user_id,reference,status,priority,issue_date,start_date,expiration_date,revision,created_at
    from public.documental_records r
    where category=p_category
      and (coalesce(p_search,'')='' or concat_ws(' ',title,reference,authority,department,related_employee_name,provider_name,study_type,periodicity) ilike '%'||p_search||'%')
...
create function public.documental_safety_context() returns jsonb
language plpgsql stable security definer set search_path = public, pg_temp as $$
begin
  if not public.documental_can_manage() then raise exception 'No tienes acceso a Gestión Documental.' using errcode='42501'; end if;
  return public.documental_context() || jsonb_build_object('employees',coalesce(
    (select jsonb_agg(jsonb_build_object('id',e.id,'label',e.full_name) order by lower(e.full_name),e.id)
      from public.employees e where e.is_active),'[]'::jsonb));
end; $$;
revoke all on function public.documental_safety_context() from public;
grant execute on function public.documental_safety_context() to authenticated;

-- Existing context and list signatures remain compatible with previous clients.
