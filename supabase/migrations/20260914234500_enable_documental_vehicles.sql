...
alter table public.documental_records
  ...
  add column vehicle_label text not null default '';
create index documental_records_vehicle_idx on public.documental_records(vehicle_id) where vehicle_id is not null;
...
begin
  ...
  if v_category not in ('documentacion-legal','permisos-y-tramites','seguridad-e-higiene','medio-ambiente','vehiculos') then
  ...
  if v_category in ('permisos-y-tramites','seguridad-e-higiene','medio-ambiente','vehiculos') then
...
begin
  ...
  if p_category not in ('documentacion-legal','permisos-y-tramites','seguridad-e-higiene','medio-ambiente','vehiculos') or p_category is null then raise exception 'Categoría no habilitada.' using errcode='23514'; end if;
...
-- Only expose the identity needed by the documental picker; include all operational statuses.
create or replace function public.documental_vehicle_context() returns jsonb
language plpgsql stable security definer set search_path=public,pg_temp as $$
declare result jsonb;
begin
  if not public.documental_can_manage() then raise exception 'No tienes acceso a Gestión Documental.' using errcode='42501'; end if;
  result:=public.documental_context();
  return result || jsonb_build_object('vehicles',coalesce((
    select jsonb_agg(jsonb_build_object('id',q.id,'label',q.label) order by lower(q.label),q.id)
    from (select v.id,coalesce(nullif(concat_ws(' · ',nullif(btrim(v.code),''),
        case when btrim(coalesce(v.nickname,''))<>btrim(coalesce(v.code,'')) then nullif(btrim(v.nickname),'') end,
        nullif(btrim(v.plate),'')),''),v.id::text) as label from public.vehicles v) q
  ),'[]'::jsonb));
end; $$;
revoke all on function public.documental_vehicle_context() from public;
grant execute on function public.documental_vehicle_context() to authenticated;
