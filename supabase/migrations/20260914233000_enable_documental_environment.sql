...
alter table public.documental_records
  ...
  add column installation_name text not null default '';
...
begin
  ...
  if v_category not in ('documentacion-legal','permisos-y-tramites','seguridad-e-higiene','medio-ambiente') then
  ...
  if v_category in ('permisos-y-tramites','seguridad-e-higiene','medio-ambiente') then
  ...
  insert into public.documental_records(id,category,title,document_type,responsible_user_id,department,authority,reference,status,priority,
    issue_date,start_date,expiration_date,observations,next_action,revision,created_by,updated_by,progress_percentage,related_employee_id,related_employee_name,study_type,provider_name,periodicity,authorization_number,installation_name)
  values(p_id,v_category,btrim(p_record->>'title'),p_record->>'document_type',(p_record->>'responsible_user_id')::uuid,
    ...
    nullif(p_record->>'expiration_date','')::date,coalesce(p_record->>'observations',''),coalesce(p_record->>'next_action',''),v_revision,auth.uid(),auth.uid(),v_progress,v_employee,v_employee_name,v_study,v_provider,v_periodicity,v_authorization,v_installation)
  on conflict(id) do update set title=excluded.title,document_type=excluded.document_type,responsible_user_id=excluded.responsible_user_id,
    ...
    authorization_number=excluded.authorization_number,installation_name=excluded.installation_name,revision=v_revision,updated_at=now(),updated_by=auth.uid();
...
begin
  ...
  if p_category not in ('documentacion-legal','permisos-y-tramites','seguridad-e-higiene','medio-ambiente') or p_category is null then raise exception 'Categoría no habilitada.' using errcode='23514'; end if;
  with filtered as materialized (
    select id,category,title,document_type,authority,department,progress_percentage,related_employee_id,related_employee_name,study_type,provider_name,periodicity,authorization_number,installation_name,responsible_user_id,reference,status,priority,issue_date,start_date,expiration_date,revision,created_at
    from public.documental_records r
    where category=p_category
      and (coalesce(p_search,'')='' or concat_ws(' ',title,reference,authority,department,related_employee_name,provider_name,study_type,periodicity,authorization_number,installation_name) ilike '%'||p_search||'%')
