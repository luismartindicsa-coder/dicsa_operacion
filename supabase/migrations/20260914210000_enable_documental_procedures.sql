...
-- Reuse the private documentary repository and version history for procedures.
-- Status and progress are independent; urgency remains derived from dates/status.
alter table public.documental_records add column progress_percentage smallint not null default 0
...
begin
  ...
  if v_category not in ('documentacion-legal','permisos-y-tramites') then
  ...
  if v_category='permisos-y-tramites' then
  ...
  values(p_id,v_category,btrim(p_record->>'title'),p_record->>'document_type',(p_record->>'responsible_user_id')::uuid,
    ...
    nullif(p_record->>'expiration_date','')::date,coalesce(p_record->>'observations',''),coalesce(p_record->>'next_action',''),v_revision,auth.uid(),auth.uid(),v_progress)
  on conflict(id) do update set title=excluded.title,document_type=excluded.document_type,responsible_user_id=excluded.responsible_user_id,
    ...
    next_action=excluded.next_action,progress_percentage=excluded.progress_percentage,revision=v_revision,updated_at=now(),updated_by=auth.uid();
...
create function public.documental_query_records(p_category text,p_search text default '',p_status text default null,p_type text default null,
...
begin
  ...
  if p_category not in ('documentacion-legal','permisos-y-tramites') or p_category is null then raise exception 'Categoría no habilitada.' using errcode='23514'; end if;
  with filtered as materialized (
    select id,category,title,document_type,authority,department,progress_percentage,responsible_user_id,reference,status,priority,issue_date,start_date,expiration_date,revision,created_at
...
-- Keep documental_list_records and its original signature for older Legal clients.
